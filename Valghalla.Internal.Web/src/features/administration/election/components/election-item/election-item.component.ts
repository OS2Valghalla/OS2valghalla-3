...
import {
  ...
  CreateElectionTaskTypeCommunicationTemplateRequest,
} from 'src/features/administration/election/models/create-election-request';
...
export class ElectionItemComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    this.subs.sink = combineLatest({
      ...
      this.setSelectedTemplate(
        this.communicationTemplates.filter(
          (t) => t.id == DefaultCommunicationTemplates.ConfirmationOfRegistrationStringId,
        )[0],
        this.formCommunicationTemplates.controls.confirmationOfRegistration_TemplateType,
        this.formCommunicationTemplates.controls.confirmationOfRegistration_Template,
      );
      this.setSelectedTemplate(
        this.communicationTemplates.filter(
          (t) => t.id == DefaultCommunicationTemplates.ConfirmationOfCancellationStringId,
        )[0],
        this.formCommunicationTemplates.controls.confirmationOfCancellation_TemplateType,
        this.formCommunicationTemplates.controls.confirmationOfCancellation_Template,
      );
      this.setSelectedTemplate(
        this.communicationTemplates.filter((t) => t.id == DefaultCommunicationTemplates.InvitationStringId)[0],
        this.formCommunicationTemplates.controls.invitation_TemplateType,
        this.formCommunicationTemplates.controls.invitation_Template,
      );
      this.setSelectedTemplate(
        this.communicationTemplates.filter((t) => t.id == DefaultCommunicationTemplates.InvitationReminderStringId)[0],
        this.formCommunicationTemplates.controls.invitationReminder_TemplateType,
        this.formCommunicationTemplates.controls.invitationReminder_Template,
      );
      this.setSelectedTemplate(
        this.communicationTemplates.filter((t) => t.id == DefaultCommunicationTemplates.TaskReminderStringId)[0],
        this.formCommunicationTemplates.controls.taskReminder_TemplateType,
        this.formCommunicationTemplates.controls.taskReminder_Template,
      );
      this.setSelectedTemplate(
        this.communicationTemplates.filter((t) => t.id == DefaultCommunicationTemplates.RetractedInvitationStringId)[0],
        this.formCommunicationTemplates.controls.retractedInvitation_TemplateType,
        this.formCommunicationTemplates.controls.retractedInvitation_Template,
      );
      this.setSelectedTemplate(
        this.communicationTemplates.filter((t) => t.id == DefaultCommunicationTemplates.RemovedFromTaskStringId)[0],
        this.formCommunicationTemplates.controls.removedFromTask_TemplateType,
        this.formCommunicationTemplates.controls.removedFromTask_Template,
      );
      this.setSelectedTemplate(
        this.communicationTemplates.filter((t) => t.id == DefaultCommunicationTemplates.RemovedByValidationStringId)[0],
        this.formCommunicationTemplates.controls.removedByValidation_TemplateType,
        this.formCommunicationTemplates.controls.removedByValidation_Template,
      );
      ...
      this.taskTypes.forEach((taskType) => {
        ...
        taskType.removedFromTask_Template = undefined;
        taskType.removedByValidation_Template = undefined;
      });
  ...
  setSelectedTemplate(
    selectedTemplate: CommunicationTemplateListingItem,
    formTemplateTypeFieldName: FormControl,
    formTemplateFieldName: FormControl,
  ) {
  ...
  validateElectionDate(control: AbstractControl): ValidationErrors | null {
    if (
      !control ||
      !control.get('electionStartDate') ||
      !control.get('electionStartDate').value ||
      !control.get('electionEndDate') ||
      !control.get('electionEndDate').value ||
      !control.get('electionDate') ||
      !control.get('electionDate').value
    ) {
    ...
    if (
      control.get('electionStartDate').value.valueOf() > control.get('electionDate').value.valueOf() ||
      control.get('electionDate').value.valueOf() > control.get('electionEndDate').value.valueOf()
    ) {
  ...
  createElection(event: WizardEvent) {
    ...
    const createRequest: CreateElectionRequest = {
      ...
      confirmationOfCancellationCommunicationTemplateId:
        this.formCommunicationTemplates.value.confirmationOfCancellation_Template.id,
      ...
      retractedInvitationCommunicationTemplateId:
        this.formCommunicationTemplates.value.retractedInvitation_Template?.id,
      removedFromTaskCommunicationTemplateId: this.formCommunicationTemplates.value.removedFromTask_Template?.id,
      removedByValidationCommunicationTemplateId:
        this.formCommunicationTemplates.value.removedByValidation_Template?.id,
  ...
  hasSpecificTemplate(taskType: TaskTypeWithTemplates) {
    if (
      (taskType.confirmationOfRegistration_Template && taskType.confirmationOfRegistration_Template.id) ||
  ...
  getDeleteButtonTooltip = () => {
    ...
    return this.item.id == localStorage.getItem(SELECTED_ELECTION_TO_WORK_ON)
      ? 'administration.election.messages.cannot_delete_selected_election'
      : this.item.active
      ? 'administration.election.messages.cannot_delete_active_election'
      : '';
  };
}
